...
async def extract_rating(page):
    try:
        rating_selectors = [
            'div.uYNZm', 
            'div[data-ved] div.EbPs6b',
            'div.jdgnK',  # Ek bir seçici
            'span[data-ved] span'  # Başka bir seçici
        ]
    ...
    try:
        ...
        for index, product_name in enumerate(product_names, start=1):
            ...
            if json_filename and os.path.exists(json_filename):
                ...
                combined_data[f'product{index}'] = {
                    'name': product_name,
                    'google_data': {'error': f"Data not found for {product_name}"}
                }
